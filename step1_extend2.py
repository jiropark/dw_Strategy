"""3단계 추가 수집: 페이지 81~120 (더 오래된 데이터)"""
import os
import time
import ssl
import json
...
print(f"\n[1/3] {len(tickers)}종목 3단계 추가 수집 (페이지 81~120)...")
success = 0
for idx, (ticker, name) in enumerate(zip(tickers, names)):
    print(f"  [{idx+1:3d}/{len(tickers)}] {ticker} ({name}) ...", end="", flush=True)
    ...
    try:
        ...
        new_df = get_naver_ohlcv_pages(ticker, 81, 120)
        if len(new_df) == 0:
            print(f" 추가없음 ({old_count}일)")
            continue
        ...
        print(f" {old_count}→{len(merged)}일 (+{added})")
        success += 1
    except Exception as e:
        print(f" FAIL ({e})")
    time.sleep(0.3)

print(f"\n종목 수집 완료: {success}종목 확장")

# KOSPI 지수 추가
print(f"\n[2/3] KOSPI 지수 추가 (페이지 101~150)...")
SESSION = make_session()
kospi_extra = []
for page in range(101, 151):
...
if kospi_extra:
    ...
    print(f"KOSPI: {old_len}→{len(merged_kospi)}일 (+{len(merged_kospi)-old_len})")
...
print(f"  전체: {dates[0].date()} ~ {dates[-1].date()} ({n}일)")
